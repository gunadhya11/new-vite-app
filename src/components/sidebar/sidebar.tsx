import Chevron from "./undraw_chevrons.svg";
import { AnimatePresence, motion, useAnimate } from "framer-motion";

import { useState } from "react";

const linkArr: { text: string; icon: string }[] = [
  { text: "link1 link1 link1 ", icon: "car" },
  { text: "link1 link1 link1 ", icon: "briefcase" },
  { text: "link1 link1 link1 ", icon: "book-atlas" },
  { text: "link1 link1 link1 ", icon: "spa" },
  { text: "link1 link1 link1 ", icon: "suitcase-rolling" },
];
const Sidebar = () => {
  ...
  const handleSidebarToggle = () => {
    if (collapse) {
      animate([
        [scope.current, { width: 75 }, { duration: 0.5, type: "spring" }],
        [".toggle-button img", { rotate: 180 }, { duration: 0.15 }],
      ]);
    } else {
      animate([
        [scope.current, { width: 175 }, { duration: 0.5 }],
        [".toggle-button img", { rotate: 0 }, { duration: 0.15 }],
      ]);
    }
  ...
  return (
    <motion.div
      className="w-44 h-screen border-r-2 bg-default-500 px-4 relative bg-default pt-6 pb-14 sidebar"
      ref={scope}
      layout
    >
      <div className="pb-16 logo flex">
        <motion.svg
          id="logo-83"
          width="40"
          height="35"
        ...
        {collapse && (
          <AnimatePresence>
            <motion.div
              ...
              animate={{ opacity: 1, width: 100, type: "spring" }}
              exit={{ opacity: 0, width: 0 }}
              className="text-secondary text-2xl font-bold ml-2"
            >
              {" "}
              name
            </motion.div>
      ...
      <button
        ...
        className="toggle-button absolute border-none top-16 -right-3 h-6 w-6 bg-default focus:border-none border-gray-700 rounded-full text-white flex justify-center items-center hover:bg-tirtiary"
      >
        <i
          className={`fa-solid ${
            collapse ? "fa-chevron-left" : "fa-chevron-right"
          }`}
        ></i>
      </button>
      <ul>
        {linkArr.map((item: { text: string; icon: string }, index: number) => {
          return (
            <li
              className="p-2 py-3 hover:bg-secondary hover:text-white text-secondary rounded-lg flex  items-center"
              key={index}
            >
              <motion.i
                layout
                className={`w-10 h-8 fa-${item.icon} fa-lg fa-solid flex items-center justify-center`}
              ></motion.i>
              {collapse && (
                <AnimatePresence>
                  <motion.div
                    ...
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                    className="ml-2 text-left flex-wrap"
                  >
                    {item.text}
                  </motion.div>
                </AnimatePresence>
